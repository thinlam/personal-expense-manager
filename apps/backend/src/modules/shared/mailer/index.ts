...
export async function sendVerifyEmailOtpEmail(to: string, otp: string) {
  const APP_NAME = "Expense Manager";
  const BRAND_COLOR = "#10B981";
  const subject = `${APP_NAME} — Xác minh email đăng ký`;

  const text = [
    `Xin chào,`,
    ``,
    `Bạn đang xác minh email để hoàn tất đăng ký tài khoản trên ${APP_NAME}.`,
    ``,
    `Mã OTP của bạn: ${otp}`,
    `Mã có hiệu lực trong 10 phút.`,
    ``,
            ...
            <tr>
              <td style="background:#ffffff;border-radius:16px;box-shadow:0 10px 30px rgba(15,23,42,0.08);overflow:hidden;">
                ...
                <div style="padding:28px 28px 10px 28px;">
                  ...
                  <div style="font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.7;color:#475569;margin-top:10px;">
                    <b style="color:#0f172a;">Lưu ý bảo mật:</b> Không chia sẻ mã này với bất kỳ ai.
                  </div>
